...
#     plt.plot(df['MA_short'], label='MA50')
#     plt.plot(df['MA_long'], label='MA100')
...
#     plt.title("Moving Average Strategy (MA50 vs MA100)")
#     plt.legend()
#     plt.show()
    
    
# df_ma = moving_average_strategy(df)
# plot_ma_strategy(df_ma)
...
#     plt.plot(df['Mean'], label='Rolling Mean')
    
#     plt.title("Mean Reversion Strategy")
#     plt.legend()
#     plt.show()
    
# df_mr = mean_reversion_strategy(df)
# plot_mean_reversion(df_mr)
...
#     plt.plot(df['Close'], label='Price')
#     plt.plot(df['High_roll'], linestyle='--', label='Resistance')
#     plt.plot(df['Low_roll'], linestyle='--', label='Support')
    
#     plt.title("Breakout Strategy")
#     plt.legend()
#     plt.show()
    
# df_bo = breakout_strategy(df)
# plot_breakout(df_bo)


def plot_all_strategies(df):
    plt.figure(figsize=(14,6))
    
    strategies = {
        "MA": moving_average_strategy,
        "RSI": momentum_strategy,
        "MeanRev": mean_reversion_strategy,
        "Breakout": breakout_strategy
    }
    ...
    for name, func in strategies.items():
        temp = func(df)
        temp = backtest(temp)
        plt.plot(temp['Cumulative_Strategy'], label=name)
    
    # ✅ FIX: create market signal
    df_market = df.copy()
    df_market['Signal'] = 1   # always invested
    
    df_market = backtest(df_market)
    
    plt.plot(df_market['Cumulative_Market'], label='Market', linestyle='--')
    
    plt.title("Strategy Comparison vs Market")
    plt.legend()
    plt.show()
    
plot_all_strategies(df)
...
def save_table_image(df):
    ...
    plt.savefig("strategy_table.png", bbox_inches='tight')
    plt.show()
save_table_image(results_df)